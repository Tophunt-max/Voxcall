 ...
 */
// ── Pacing philosophy ───────────────────────────────────────────────────────
// Early rungs must feel FAST so a new host is rewarded within their first
// session, then the curve steepens. Crucially, the early gates use metrics
// that accumulate on EVERY call — total_minutes + unique_callers — NOT
// review_count (rated calls), because ratings are sparse and gating L2 on "50
// rated calls" made the old ladder feel impossible. Ratings/earnings only gate
// the higher, aspirational rungs where a host has enough history for them to be
// meaningful. Rewards + earning-share + rate caps climb every rung.
export const DEFAULT_LEVEL_CONFIG: LevelDef[] = [
  { level: 1, name: 'Newcomer', badge: '🌱', color: '#6B7280', min_calls: 0,   min_rating: 0.0, min_minutes: 0,    min_earnings: 0,     criteria: [],                                                                                                                                                                                                                    coin_reward: 0,    description: 'New here — start taking calls',              perks: { max_rate: 55, max_audio_rate: 35, max_video_rate: 55, random_audio_rate: 25, random_video_rate: 40, earning_share: 0.70, rank_boost: 0 } },
  { level: 2, name: 'Riser',    badge: '⭐', color: '#10B981', min_calls: 0,   min_rating: 0.0, min_minutes: 5,    min_earnings: 0,     criteria: [{ metric: 'total_minutes', op: '>=', value: 5 }, { metric: 'unique_callers', op: '>=', value: 1 }],                                                                                                                     coin_reward: 50,   description: 'Your first calls — welcome aboard!',        perks: { max_rate: 85, max_audio_rate: 60, max_video_rate: 85, random_audio_rate: 25, random_video_rate: 40, earning_share: 0.70, rank_boost: 1 } },
  { level: 3, name: 'Star',     badge: '🌟', color: '#F59E0B', min_calls: 3,   min_rating: 3.5, min_minutes: 60,   min_earnings: 0,     criteria: [{ metric: 'total_minutes', op: '>=', value: 60 }, { metric: 'unique_callers', op: '>=', value: 5 }, { metric: 'review_count', op: '>=', value: 3 }, { metric: 'rating', op: '>=', value: 3.5 }],                            coin_reward: 150,  description: 'Building a real audience',                  perks: { max_rate: 120, max_audio_rate: 90, max_video_rate: 120, random_audio_rate: 25, random_video_rate: 40, earning_share: 0.72, rank_boost: 2 } },
  { level: 4, name: 'Expert',   badge: '🔥', color: '#EF4444', min_calls: 10,  min_rating: 4.0, min_minutes: 300,  min_earnings: 3000,  criteria: [{ metric: 'total_minutes', op: '>=', value: 300 }, { metric: 'unique_callers', op: '>=', value: 20 }, { metric: 'review_count', op: '>=', value: 10 }, { metric: 'rating', op: '>=', value: 4.0 }, { metric: 'total_earnings', op: '>=', value: 3000 }],                       coin_reward: 400,  description: 'Proven, consistent performer',              perks: { max_rate: 155, max_audio_rate: 125, max_video_rate: 155, random_audio_rate: 25, random_video_rate: 40, earning_share: 0.74, rank_boost: 3 } },
  { level: 5, name: 'Pro',      badge: '💎', color: '#8B5CF6', min_calls: 40,  min_rating: 4.3, min_minutes: 1200, min_earnings: 15000, criteria: [{ metric: 'total_minutes', op: '>=', value: 1200 }, { metric: 'unique_callers', op: '>=', value: 60 }, { metric: 'review_count', op: '>=', value: 40 }, { metric: 'rating', op: '>=', value: 4.3 }, { metric: 'total_earnings', op: '>=', value: 15000 }, { metric: 'answer_rate', op: '>=', value: 0.70 }],  coin_reward: 800,  description: 'Loyal fan base & great quality',            perks: { max_rate: 185, max_audio_rate: 165, max_video_rate: 185, random_audio_rate: 25, random_video_rate: 40, earning_share: 0.77, rank_boost: 4 } },
  { level: 6, name: 'Elite',    badge: '👑', color: '#D97706', min_calls: 150, min_rating: 4.6, min_minutes: 3500, min_earnings: 50000, criteria: [{ metric: 'total_minutes', op: '>=', value: 3500 }, { metric: 'unique_callers', op: '>=', value: 200 }, { metric: 'review_count', op: '>=', value: 150 }, { metric: 'rating', op: '>=', value: 4.6 }, { metric: 'total_earnings', op: '>=', value: 50000 }, { metric: 'answer_rate', op: '>=', value: 0.80 }, { metric: 'kyc_verified', op: '==', value: 1 }], coin_reward: 2000, description: 'Top-tier, verified & in demand',            perks: { max_rate: 200, max_audio_rate: 200, max_video_rate: 200, random_audio_rate: 25, random_video_rate: 40, earning_share: 0.80, rank_boost: 6 } },
];
...
function buildRecommendedLadder(): LevelDef[] {
  ...
  // Rate caps (coins/min). Start JUST above the default rates (audio 30 /
  // video 50) at Level 1, rise in SMALL steps through Level 10, then BIGGER
  // steps to the ceiling of 200 at Level 20. Video runs a bit higher than
  // audio. Top-level max coin/min = 200.
  const audioCaps = [35, 40, 45, 51, 57, 63,  70,  77,  85,  93, 103, 113, 124, 135, 147, 160, 173, 185, 193, 200];
  const videoCaps = [55, 62, 69, 76, 83, 90,  98, 106, 114, 122, 132, 142, 152, 162, 172, 181, 189, 195, 198, 200];
  ...
  for (let i = 0; i < TIERS.length; i++) {
    ...
    const audioCap = audioCaps[i] ?? 200;
    const videoCap = videoCaps[i] ?? 200;
    out.push({
      ...
      description,
      perks: {
        max_rate: Math.max(audioCap, videoCap), max_audio_rate: audioCap, max_video_rate: videoCap,
