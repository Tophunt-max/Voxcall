...
export const DEFAULT_LEVEL_CONFIG: LevelDef[] = [
  { level: 1, name: 'Newcomer', badge: '🌱', color: '#6B7280', min_calls: 0,   min_rating: 0.0, min_minutes: 0,    min_earnings: 0,     criteria: [],                                                                                                                                                                                                                    coin_reward: 0,    description: 'New here — start taking calls',              perks: { max_rate: 200, max_audio_rate: 200, max_video_rate: 200, random_audio_rate: 25, random_video_rate: 40, earning_share: 0.70, rank_boost: 0 } },
  { level: 2, name: 'Riser',    badge: '⭐', color: '#10B981', min_calls: 0,   min_rating: 0.0, min_minutes: 5,    min_earnings: 0,     criteria: [{ metric: 'total_minutes', op: '>=', value: 5 }, { metric: 'unique_callers', op: '>=', value: 1 }],                                                                                                                     coin_reward: 50,   description: 'Your first calls — welcome aboard!',        perks: { max_rate: 260, max_audio_rate: 260, max_video_rate: 260, random_audio_rate: 25, random_video_rate: 40, earning_share: 0.70, rank_boost: 1 } },
  { level: 3, name: 'Star',     badge: '🌟', color: '#F59E0B', min_calls: 3,   min_rating: 3.5, min_minutes: 60,   min_earnings: 0,     criteria: [{ metric: 'total_minutes', op: '>=', value: 60 }, { metric: 'unique_callers', op: '>=', value: 5 }, { metric: 'review_count', op: '>=', value: 3 }, { metric: 'rating', op: '>=', value: 3.5 }],                            coin_reward: 150,  description: 'Building a real audience',                  perks: { max_rate: 320, max_audio_rate: 320, max_video_rate: 320, random_audio_rate: 25, random_video_rate: 40, earning_share: 0.72, rank_boost: 2 } },
  { level: 4, name: 'Expert',   badge: '🔥', color: '#EF4444', min_calls: 10,  min_rating: 4.0, min_minutes: 300,  min_earnings: 3000,  criteria: [{ metric: 'total_minutes', op: '>=', value: 300 }, { metric: 'unique_callers', op: '>=', value: 20 }, { metric: 'review_count', op: '>=', value: 10 }, { metric: 'rating', op: '>=', value: 4.0 }, { metric: 'total_earnings', op: '>=', value: 3000 }],                       coin_reward: 400,  description: 'Proven, consistent performer',              perks: { max_rate: 380, max_audio_rate: 380, max_video_rate: 380, random_audio_rate: 25, random_video_rate: 40, earning_share: 0.74, rank_boost: 3 } },
  { level: 5, name: 'Pro',      badge: '💎', color: '#8B5CF6', min_calls: 40,  min_rating: 4.3, min_minutes: 1200, min_earnings: 15000, criteria: [{ metric: 'total_minutes', op: '>=', value: 1200 }, { metric: 'unique_callers', op: '>=', value: 60 }, { metric: 'review_count', op: '>=', value: 40 }, { metric: 'rating', op: '>=', value: 4.3 }, { metric: 'total_earnings', op: '>=', value: 15000 }, { metric: 'answer_rate', op: '>=', value: 0.70 }],  coin_reward: 800,  description: 'Loyal fan base & great quality',            perks: { max_rate: 440, max_audio_rate: 440, max_video_rate: 440, random_audio_rate: 25, random_video_rate: 40, earning_share: 0.77, rank_boost: 4 } },
...
function buildRecommendedLadder(): LevelDef[] {
  ...
  for (let i = 0; i < TIERS.length; i++) {
    ...
    // Rate cap climbs evenly from 200 (Level 1) to 500 (Level 20 =
    // ABSOLUTE_MAX_RATE) — never above 500. Same ceiling for audio + video.
    const cap = Math.min(ABSOLUTE_MAX_RATE, Math.round(200 + (300 * i) / 19));
    out.push({
