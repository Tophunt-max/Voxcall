...
app.use('/api/*', async (c, next) => {
  ...
  await Promise.all([
    ...
    ensureHostLevelSchema(c.env.DB),
  ]);
  return next();
});
...
app.route('/api/user/pass', passRouter);
