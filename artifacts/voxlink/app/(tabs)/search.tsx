  ...
  return (
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.88}>
      ...
      <View style={styles.cardActions}>
        ...
        {host.isOnline && (
          <TouchableOpacity onPress={() => router.push(`/call/outgoing?hostId=${host.id}`)} style={styles.talkNowBtn}>
  ...
  return (
    <View style={styles.container}>
      ...
      {filtered.length === 0 ? (
        ...
        >
          {filtered.map((host) => (
            <ListenerCard key={host.id} host={host} onPress={() => router.push(`/hosts/${host.id}`)} />
          ))}
