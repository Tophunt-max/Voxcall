...
function RippleRings() {
  ...
  return (
    <View style={[styles.rippleContainer, { pointerEvents: "none" } as any]}>
      {rings.map((v, i) => (
        <Animated.View key={i} style={[styles.rippleRing, { opacity: v.interpolate({ inputRange: [0, 0.15, 0.7, 1], outputRange: [0, 0.5, 0.5, 0] }), transform: [{ scale: v.interpolate({ inputRange: [0, 1], outputRange: [0.3, 1.5] }) }] }]} />
      ))}
    </View>
  );
}
...
function ListenerCard({ host, isLeft, isSpecial, delay, onReplace, onPress }: ListenerCardProps) {
  ...
  const pillRadius = isSpecial ? { borderBottomRightRadius: 42, borderTopRightRadius: 42 } : { borderBottomLeftRadius: 42, borderTopLeftRadius: 42 };
  const pillPad = isSpecial ? { paddingLeft: 32, paddingRight: 14, paddingVertical: 8 } : { paddingLeft: 20, paddingRight: 32, paddingVertical: 8 };
  return (
    <Animated.View style={{ opacity, transform: [{ scale }] }}>
      <TouchableOpacity onPress={onPress} activeOpacity={0.85} style={[styles.cardAligner, isLeft ? { alignSelf: "flex-start" } : { alignSelf: "flex-end" }]}>
        ...
        <View style={[styles.cardAvatar, avatarSide]}>
          <Image source={require("@/assets/images/avatar_placeholder.png")} style={styles.cardAvatarImg} />
        </View>
      </TouchableOpacity>
    </Animated.View>
  );
...
function CallTypeDialog({ visible, selected, onSelect, onClose }: { visible: boolean; selected: CallType; onSelect: (t: CallType) => void; onClose: () => void }) {
  return (
        ...
        <View style={styles.dialogBox}>
          ...
          {(["audio", "video"] as CallType[]).map((type) => (
            <TouchableOpacity key={type} onPress={() => { onSelect(type); onClose(); }} style={styles.dialogRow}>
              <Image source={type === "audio" ? require("@/assets/icons/ic_call_gradient.png") : require("@/assets/icons/ic_chat_video.png")} style={styles.dialogIcon} resizeMode="contain" />
...
function MatchRipple() {
  ...
  useEffect(() => {
    const anim = Animated.parallel(rings.map((v, i) => Animated.loop(Animated.sequence([Animated.delay(i * 400), Animated.timing(v, { toValue: 1, duration: 3000, useNativeDriver: false }), Animated.timing(v, { toValue: 0, duration: 0, useNativeDriver: false })]))));
    anim.start();
    return () => anim.stop();
  }, []);
  return (
...
function MatchFoundScreen({ host, callType, onAccept, onDecline }: { host: (typeof MOCK_HOSTS)[0]; callType: CallType; onAccept: () => void; onDecline: () => void }) {
  const scale = useRef(new Animated.Value(0.7)).current;
  useEffect(() => { Animated.spring(scale, { toValue: 1, tension: 55, friction: 8, useNativeDriver: false }).start(); }, []);
  return (
...
export default function RandomScreen() {
  ...
  const handleAccept = () => {
    ...
    setPhase("idle");
    router.push(callType === "video" ? `/call/video-call?hostId=${matchedHost.id}` : `/call/audio-call?hostId=${matchedHost.id}`);
  };
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={[styles.header, { paddingTop: topPad + 14 }]}>
        ...
        <TouchableOpacity onPress={() => router.push("/payment/checkout")} style={styles.coinWidget} activeOpacity={0.85}>
      ...
      <View style={[styles.bottomBtns, { paddingBottom: insets.bottom + 16 }]}>
        <TouchableOpacity onPress={() => setDialogVisible(true)} style={styles.callTypeBtn} activeOpacity={0.85}>
          <Image source={callType === "audio" ? require("@/assets/icons/ic_call_gradient.png") : require("@/assets/icons/ic_chat_video.png")} style={styles.callTypeBtnIcon} resizeMode="contain" />
        ...
        <TouchableOpacity onPress={handleFindMatch} disabled={phase === "searching"} activeOpacity={0.85} style={styles.randomBtnWrap}>
          <LinearGradient colors={GRAD} style={styles.randomBtn} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
            <Text style={styles.randomBtnTxt}>{phase === "searching" ? "Searching..." : "Random Match"}</Text>
          </LinearGradient>
        </TouchableOpacity>
      </View>

      <CallTypeDialog visible={dialogVisible} selected={callType} onSelect={setCallType} onClose={() => setDialogVisible(false)} />
      {phase === "found" && matchedHost && (
        <MatchFoundScreen host={matchedHost} callType={callType} onAccept={handleAccept} onDecline={handleDecline} />
      )}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  callTypeBtn: { flexDirection: "row", alignItems: "center", backgroundColor: "#fff", borderRadius: 30, paddingVertical: 10, paddingHorizontal: 20, gap: 8, shadowColor: "#000", shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.1, shadowRadius: 18, elevation: 6, marginBottom: 0 },
  ...
  matchName: { fontSize: 22, fontFamily: "Poppins_700Bold", color: "#111329" },
  matchId: { fontSize: 13, fontFamily: "Poppins_400Regular", color: "#757396" },
  matchTopicsRow: { flexGrow: 0, marginVertical: 8 },
  ...
  matchBtns: { flexDirection: "row", gap: 40, marginTop: 16 },
  matchDecline: { width: 64, height: 64, borderRadius: 32, backgroundColor: "#FF025F", alignItems: "center", justifyContent: "center" },
  matchAccept: { width: 64, height: 64, borderRadius: 32, alignItems: "center", justifyContent: "center" },
