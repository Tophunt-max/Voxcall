  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#fff" }}>
      <LinearGradient colors={[DARK, "#2D3057"]} style={[s.header, { paddingTop: insets.top + 10 }]}>
        <TouchableOpacity onPress={() => router.canGoBack() ? router.back() : router.replace("/auth/role-select")} style={s.backBtn}>
      ...
      >
        ...
        {status === "approved" && (
          <TouchableOpacity
            style={s.ctaBtn}
            onPress={() => router.replace("/screens/host")}
            activeOpacity={0.85}
          >
        ...
        {(status === "rejected" || status === "not_applied") && (
          <TouchableOpacity
            style={s.ctaBtn}
            onPress={() => router.push("/auth/host-register")}
            activeOpacity={0.85}
          >
