...
// ─── Shared promotional banner card ───────────────────────────────────────────
// One polished, production-grade design used by every banner slider (home,
// search, wallet). Each admin banner gets its own look via:
//   • bg_color → gradient_to  (a two-tone gradient; gradient_to falls back to a
//                              derived darker shade so single-colour rows still
//                              look designed)
//   • icon                    (an emoji shown on the right when there's no image)
//   • image_url               (full-bleed image with a dark scrim for legibility)
// The card is self-contained (fixed height, own radius/shadow) — sliders just
// pass the slide width and an onPress.
...
// Lighten (+) or darken (-) a #rrggbb hex by `percent` (0..100).
...
export default function PromoBannerCard({
  ...
  const c1 = (banner.bg_color || "#7C3AED").trim();
  ...
  return (
    ...
    >
      ...
      {/* Decorative translucent blobs give depth on flat/gradient cards. */}
